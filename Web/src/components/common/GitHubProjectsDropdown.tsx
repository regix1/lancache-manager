...
function getFireworkColors(): string[] {
  ...
  if (colors.length === 0) {
    ...
    if (themeStyle?.textContent) {
      for (let i = 1; i <= 8; i++) {
        const match = themeStyle.textContent.match(
          new RegExp(`--theme-firework-${i}:\\s*(#[a-fA-F0-9]{6}|#[a-fA-F0-9]{3})`)
        );
        if (match?.[1]) {
...
const FirecrackerSVG: React.FC = () => (
  <svg height="100%" width="100%" viewBox="0 0 479.217 479.217" xmlns="http://www.w3.org/2000/svg">
    {/* Main body - primary */}
    <path
      d="m197.234 340.137-68.956-68.957c-11.425-11.425-11.425-29.949 0-41.374l17.542-17.542 50.444-37.529 33.543-46.457 77.759-77.759c11.425-11.425 29.949-11.425 41.374 0l68.956 68.956c11.425 11.425 11.425 29.949 0 41.374l-179.288 179.288c-11.425 11.425-29.949 11.425-41.374 0z"
    ...
    <path
      d="m417.895 119.476-68.956-68.956c-11.425-11.425-29.949-11.425-41.374 0l-22.177 22.177c8.599-8.598 22.539-8.598 31.138 0l51.897 51.897c8.599 8.599 8.599 22.539 0 31.138l-134.933 134.931c-8.599 8.599-22.54 8.599-31.138 0l-51.897-51.897c-8.599-8.599-8.599-22.539 0-31.138l-22.177 22.177c-11.425 11.425-11.425 29.949 0 41.374l22.89 22.89 46.067 41.172c11.425 11.425 29.949 11.425 41.374 0l46.067-41.172 133.22-133.22c11.424-11.424 11.424-29.948-.001-41.373z"
      fill="var(--theme-primary)"
    />
    {/* Accent detail - primary light */}
    <path
      d="m229.046 294.07c-6.854 4.023-15.396 4.023-22.251 0h-55.628l46.067 46.067c11.425 11.425 29.949 11.425 41.374 0l46.067-46.067z"
      fill="var(--theme-primary)"
      style={{ opacity: 0.5 }}
    />
    {/* Wing shadow - primary light */}
    <path
      d="m417.895 119.476-44.759-44.759v53.562h-1.756c5.479 8.524 4.5 19.996-2.958 27.454l-56.533 56.533h54.591l51.415-51.415c11.425-11.426 11.425-29.95 0-41.375z"
      fill="var(--theme-primary)"
      style={{ opacity: 0.5 }}
    />
    {/* Tail - primary */}
    <path
      d="m433.103.296-140.668 19.303c-25.451 3.492-35.916 34.601-17.751 52.766l121.366 121.365c18.165 18.165 49.273 7.7 52.766-17.751l19.302-140.668c2.797-20.385-14.63-37.812-35.015-35.015z"
      fill="var(--theme-primary)"
      style={{ opacity: 0.85 }}
    />
...
const Firework: React.FC<FireworkProps> = ({ startX, startY, direction, onComplete }) => {
  ...
  useEffect(() => {
    ...
    const dotData: { x: number; y: number; spawnTime: number; active: boolean }[] = [];
...
const GitHubProjectsDropdown: React.FC<GitHubProjectsDropdownProps> = ({ iconOnly = false }) => {
  ...
  const [fireworks, setFireworks] = useState<
    { x: number; y: number; direction: 'left' | 'center' | 'right' }[]
  >([]);
  ...
  const fireworkDirections = useRef<('left' | 'center' | 'right')[]>(['left', 'center', 'right']);
