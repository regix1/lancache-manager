...
export interface IpChipProps {
   ...
   * reads as a sentence fragment once a verb is put in front of it.
   */
   ...
   * alignment a monospace face buys on an address is not spent on prose.
   */
  mono?: boolean;
  /**
   * What the hover tooltip shows. Defaults to `address`, so a chip whose `address` prop has been
   * swapped for a name can still put the raw address on hover.
   */
  tooltip?: string;
  /**
   * A short qualifier shown after the label, for a chip whose address needs a word of explanation
   * to be read correctly, such as one typed in by hand that no download has ever come from.
   */
  note?: string;
  className?: string;
}
