...
/**
 * Type guard to check if a notification is currently running.
 * @param notification - The notification to check
 * @returns True if the notification status is 'running'
 */
export const isRunningNotification = (
  notification: UnifiedNotification
): notification is UnifiedNotification & { status: 'running' } => {
  return notification.status === 'running';
};

/**
 * Type guard to check if a notification has completed successfully.
 * @param notification - The notification to check
 * @returns True if the notification status is 'completed'
 */
export const isCompletedNotification = (
  notification: UnifiedNotification
): notification is UnifiedNotification & { status: 'completed' } => {
  return notification.status === 'completed';
};

/**
 * Type guard to check if a notification has failed.
 * @param notification - The notification to check
 * @returns True if the notification status is 'failed'
 */
export const isFailedNotification = (
  notification: UnifiedNotification
): notification is UnifiedNotification & { status: 'failed' } => {
  return notification.status === 'failed';
};

/**
 * Type guard to check if a notification is dismissible (completed or failed).
 * @param notification - The notification to check
 * @returns True if the notification can be dismissed
 */
export const isDismissibleNotification = (
  notification: UnifiedNotification
): notification is UnifiedNotification & { status: 'completed' | 'failed' } => {
  return notification.status === 'completed' || notification.status === 'failed';
};

/**
 * Type guard to check if a notification was cancelled.
 * @param notification - The notification to check
 * @returns True if the notification was cancelled
 */
export const isCancelledNotification = (notification: UnifiedNotification): boolean => {
  return notification.details?.cancelled === true;
};
