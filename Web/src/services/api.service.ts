...
class ApiService {
  ...
  static async getStreamLogInfo(): Promise<Array<{
    datasource: string;
    logsPath: string;
    ...
    try {
      const res = await fetch(`${API_BASE}/streamlogs/info`, this.getFetchOptions());
      return await this.handleResponse(res);
    } catch (error) {
      console.error('getStreamLogInfo error:', error);
      throw error;
    }
  }

  // Get stream session counts by datasource
  static async getStreamSessionCounts(): Promise<Array<{
    datasource: string;
    sessionCount: number;
    ...
    try {
      const res = await fetch(`${API_BASE}/streamlogs/session-counts`, this.getFetchOptions());
      return await this.handleResponse(res);
    } catch (error) {
      console.error('getStreamSessionCounts error:', error);
      throw error;
    }
  }

  // Start stream log processing
  static async startStreamProcessing(): Promise<{ status: string; message: string }> {
    try {
      const res = await fetch(`${API_BASE}/streamlogs/process`, this.getFetchOptions({
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      }));
      return await this.handleResponse(res);
    } catch (error) {
      console.error('startStreamProcessing error:', error);
      throw error;
    }
  }

  // Get stream processing status
  static async getStreamProcessingStatus(): Promise<{
    ...
    try {
      const res = await fetch(`${API_BASE}/streamlogs/processing-status`, this.getFetchOptions());
      return await this.handleResponse(res);
    } catch (error) {
      console.error('getStreamProcessingStatus error:', error);
      throw error;
    }
  }

  // Clear all stream sessions
  static async clearAllStreamSessions(): Promise<{ success: boolean; message: string; sessionsCleared: number }> {
    try {
      const res = await fetch(`${API_BASE}/streamlogs/sessions`, this.getFetchOptions({
        method: 'DELETE'
      }));
      return await this.handleResponse(res);
    } catch (error) {
      console.error('clearAllStreamSessions error:', error);
      throw error;
    }
  }

  // Clear stream sessions for a specific datasource
  static async clearStreamSessionsForDatasource(datasourceName: string): Promise<{ success: boolean; message: string; sessionsCleared: number }> {
    try {
      const res = await fetch(`${API_BASE}/streamlogs/sessions/${encodeURIComponent(datasourceName)}`, this.getFetchOptions({
        method: 'DELETE'
      }));
      return await this.handleResponse(res);
    } catch (error) {
      console.error('clearStreamSessionsForDatasource error:', error);
      throw error;
    }
  }

  // Get stream sessions (paginated)
  static async getStreamSessions(page: number = 1, pageSize: number = 50, datasource?: string): Promise<{
    sessions: Array<{
      id: number;
      clientIp: string;
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/streamlogs/sessions?${params}`, this.getFetchOptions());
      return await this.handleResponse(res);
    } catch (error) {
      console.error('getStreamSessions error:', error);
      throw error;
    }
  }

  // Get speed statistics summary
  static async getStreamSpeedStats(): Promise<{
    ...
    try {
      const res = await fetch(`${API_BASE}/streamlogs/speed-stats`, this.getFetchOptions());
      return await this.handleResponse(res);
    } catch (error) {
      console.error('getStreamSpeedStats error:', error);
      throw error;
    }
  }

  // Get stream log positions for all datasources
  static async getStreamLogPositions(): Promise<Array<{
    datasource: string;
    position: number;
    totalLines: number;
    logPath: string;
    ...
    try {
      const res = await fetch(`${API_BASE}/streamlogs/positions`, this.getFetchOptions());
      return await this.handleResponse(res);
    } catch (error) {
      console.error('getStreamLogPositions error:', error);
      throw error;
    }
  }

  // Reset stream log position for all datasources
  static async resetStreamLogPosition(toBeginning: boolean = true): Promise<{ message: string; position: number }> {
    try {
      const res = await fetch(`${API_BASE}/streamlogs/position`, this.getFetchOptions({
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ position: toBeginning ? 0 : null })
      }));
      return await this.handleResponse(res);
    } catch (error) {
      console.error('resetStreamLogPosition error:', error);
      throw error;
    }
  }

  // Reset stream log position for a specific datasource
  static async resetStreamLogPositionForDatasource(datasourceName: string, toBeginning: boolean = true): Promise<{ message: string; position: number }> {
    try {
      const res = await fetch(`${API_BASE}/streamlogs/position/${encodeURIComponent(datasourceName)}`, this.getFetchOptions({
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ position: toBeginning ? 0 : null })
      }));
      ...
      console.error('resetStreamLogPositionForDatasource error:', error);
      throw error;
    }
  }
}
