...
预填充会自动检测此表中几乎所有内容的正确值。**为确保预填充可靠运行，应将 `Prefill__LancacheIp` 设置为缓存服务器的 IP**——这会让预填充通过 IP 与缓存通信，完全不再依赖 DNS。对于战网强烈建议设置此项（暴雪的 CDN 域名通常不在 lancache DNS 中，可能导致预填充挂起），对于任何非标准 DNS 设置也是如此。未设置时，管理器会自动检测正在运行的 lancache 服务器并为你注入：先从 lancache 容器本身读取 `LANCACHE_IP`，再对候选地址（lancache 容器、面向主机网络缓存的 Docker 网桥网关、`host.docker.internal`）进行心跳探测，只会注入确实以 lancache 身份应答的 IP。显式设置此项仍是有保证的方式，并会完全跳过检测。仅在自动检测出错时才需要其他变量。
...
| 主机网络，主机的 DNS 未将 CDN 路由到你的缓存 | 通常无需设置——缓存会通过网桥网关自动检测并经心跳验证；若网络面板仍有警告，再设置 `Prefill__LancacheIp=<你的缓存 IP>` |
...
| `host` | 未设置 | （任意） | 通常没问题。管理器会自动检测缓存（经心跳验证，包括通过网桥网关）并注入 `LANCACHE_IP`；只有在没有任何候选通过验证时才依赖主机的 DNS。无论如何 DnsIp 都会被静默丢弃（Docker 限制）。 |
