...
[LANCache](https://lancache.net/) 的 Web 管理界面。实时查看下载进度、查看已缓存内容、追踪节省的带宽，以及在下次 LAN 聚会前预填充 Steam、Epic 和 Battle.net 游戏。Web 界面支持英语和简体中文。

**功能一览：**

- **实时仪表盘**——实时下载、节省的带宽、热门客户端、服务分析
- **缓存浏览器**——查看每个已缓存的游戏，包括封面图、大小和按客户端的历史记录
- **预填充**——在客人到来*之前*将 Steam、Epic 和 Battle.net 游戏下载到你的缓存
...
- [预填充（Steam、Epic 和 Battle.net）](#prefill-steam--epic)
- [自定义主题](#custom-themes)
...
- Docker 套接字是可选的——只有 nginx 日志轮转和 Steam/Epic 预填充需要它。
...
*选择 Steam、Epic 或 Battle.net 开始预填充会话*
...
预填充会自动检测此表中几乎所有内容的正确值。一个值得了解的变量是 **`Prefill__LancacheIp`**——将其设置为缓存服务器的 IP，预填充就完全不再依赖 DNS。仅在自动检测出错时才需要其他变量。
...
services:
  lancache-manager:
    ...
    environment:
      ...
      # 预填充（Steam、Epic 和 Battle.net） - 参见 配置 > 预填充 获取完整参考
...
## 预填充（Steam、Epic 和 Battle.net）

预填充会在用户连接**之前**将游戏下载到你的缓存中。当客人到来时，每个安装都从你的缓存读取，而不是公共互联网——全 LAN 速度，无带宽瓶颈。

Steam、Epic 和 Battle.net 各自在自己的容器中运行，因此你可以同时预填充它们而不会相互干扰。进度实时流式传输到 UI。
...
1. 打开**预填充**标签页，选择 **Steam**、**Epic Games** 或 **Battle.net**
2. 登录（Steam 使用 Steam Guard，Epic 使用 OAuth；Battle.net 无需登录）
3. 从你的库中选择游戏
4. 点击**开始**
