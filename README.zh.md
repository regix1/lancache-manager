...
[LANCache](https://lancache.net/) 的 Web 管理界面。实时查看下载进度、查看已缓存内容、追踪节省的带宽，以及在下次 LAN 聚会前预填充 Steam 和 Epic 游戏。Web 界面支持英语和简体中文。

**功能一览：**

- **实时仪表盘**——实时下载、节省的带宽、热门客户端、服务分析
- **缓存浏览器**——查看每个已缓存的游戏，包括封面图、大小和按客户端的历史记录
- **预填充**——在客人到来*之前*将 Steam 和 Epic 游戏下载到你的缓存
...
- [预填充（Steam 和 Epic）](#prefill-steam--epic)
- [自定义主题](#custom-themes)
...
Steam 和 Epic 各自在自己的容器中运行，因此你可以同时预填充两者而不会相互干扰。进度实时流式传输到 UI。
