...
[LANCache](https://lancache.net/) 的 Web 管理界面。无需打开终端，就能看清缓存的运行状况并对其进行操作。支持英语和简体中文。

仪表板是实时视图：节省的带宽、缓存命中率、热门客户端和分服务的分析数据。它背后是一个完整的缓存浏览器，收录每个已缓存游戏的封面图、大小和按客户端的历史记录。Prometheus 指标开箱即用。

它也能操作缓存：在客人到达前手动或按计划预填充 Steam、Epic、Battle.net、Riot 和 Xbox 游戏，并直接在浏览器中运行整台服务器——处理日志、清除缓存、检测损坏，出问题时检查 DNS 与缓存健康状况。
...
**已经在用内置数据库，想切换过去？** 现有数据不会自动迁移——自动迁移只覆盖旧的 SQLite 数据库，不包括内置 Postgres。请先手动导出并恢复：参见[从内置迁移到外部 PostgreSQL](docs/external-postgres-migration.md)。
...
本节是一份查询表。浏览标题，按需深入。真正涉及决策的两项设置各有专门的说明：[数据库模式](#image-variants)和[预填充网络](#prefill-network)。
