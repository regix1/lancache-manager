...
public class AppState
{
    ...
    // Null permits legacy JSON fallback; zero explicitly records no committed baseline.
    public uint? LastPicsChangeNumber { get; set; }
    // When a full PICS crawl last finished, kept apart from LastPicsCrawl because that one is
    // stamped by every successful mode, so it cannot answer "when was the last
    ...
    // Riot prefill settings (anonymous service - default-grant flag only gates feature access)
