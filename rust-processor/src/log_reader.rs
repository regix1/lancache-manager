...
mod tests {
    ...
    fn test_read_plain_file() {
        ...
        let mut reader = LogFileReader::open(temp_file.path()).unwrap();
    ...
    #[test]
    fn test_detect_gzip_extension() {
        // Just test that the path detection works
        let path = Path::new("test.log.gz");
        assert_eq!(path.extension().and_then(|e| e.to_str()), Some("gz"));
    }

    #[test]
    fn test_detect_zstd_extension() {
        let path = Path::new("test.log.zst");
