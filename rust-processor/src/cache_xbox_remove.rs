//! Xbox name-keyed game cache removal bin.
//!
//! Thin wrapper over the shared name-keyed removal core: it pins the owning service
//! to "xbox" and delegates the entire removal flow to `named_remove_core::run`.
//! All logic lives in `named_remove_core` (head) + `removal_core` (shared tail).
//!
//! Xbox identity is `Downloads.Service='xbox'` while its cache slices are hashed
//! under `LogEntries.Service='wsus'`; the name-keyed core's fallback query is
//! deliberately unconstrained on `le.Service` so the wsus-hashed URLs are still
//! returned for an `xbox` identity (cache-service split).

use anyhow::Result;

mod cache_utils;
mod cancel;
mod db;
mod log_discovery;
mod log_reader;
mod log_purge;
mod models;
mod named_remove_core;
mod parser;
mod progress_events;
mod progress_utils;
mod removal_core;
mod service_utils;
mod tact_products;

#[tokio::main]
async fn main() -> Result<()> {
    cancel::install();
    named_remove_core::run("xbox").await
}
