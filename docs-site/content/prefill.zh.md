...
配置一次，从此不必在每场活动前手动预填充。打开**管理 → 计划任务**，找到计划预填充卡片。卡片只有一个全局**运行**操作和一个全局**操作**菜单。**运行**会立即启动已启用的已保存计划。**操作**菜单用于打开“活动”和“共享容器设置”。

每个服务都会用独立的行列出其已保存计划。每一行对应一条确切的已保存计划；该行的**运行**只会启动这条计划。选择**编辑计划**会打开这条记录。保存更改并关闭编辑窗口，然后从对应行选择**运行此计划**。未保存的更改不会影响运行。

**为服务添加计划**和**复制**都会打开一份草稿。只有选择**保存计划**后，系统才会创建计划。删除已保存的计划不会停止服务容器，也不会删除已缓存的游戏；每个服务必须保留至少一条已保存计划。

每个服务只有一个由其所有计划共享的容器。通过**管理此服务的容器**启动容器并登录。计划运行只会复用已经运行的容器，绝不会自行启动容器。如果需要账号的服务尚未就绪，其计划会以“需要登录”状态跳过，其他服务仍会继续。若所有计划都被跳过，本次运行会报告为未成功，并在通知中显示原因。

**共享容器设置**为所有服务容器提供默认值。服务已有的**重启后**覆盖设置仍会优先生效。停止服务容器会将其注销并清除保存的登录。Steam、Epic 或 Xbox 的登录只能由保存该登录的 LANCache 账号复用。Battle.net 和 Riot 采用匿名方式，无需账号，但它们的共享服务容器仍必须处于运行状态。

**活动**会在同一视图中显示 Steam、Epic、Xbox、Battle.net 和 Riot。你可以查看当前下载和运行历史，也可以取消当前下载。游戏选择属于确切的已保存计划。若要删除已缓存的游戏内容，请使用**管理 → 游戏缓存移除**，不要通过删除计划来处理。

其他计划行为仍按服务区分：

- **间隔和状态。** 每条已保存计划都有自己的“运行频率”间隔。你可以暂停计划，也可以将其设置为仅在启动时运行。首次自动运行会在保存后的一个间隔之后触发；保存本身不会开始运行。
- **预设或手动选择游戏。** 预设有**全部**、**最近**和**前 N 项**三种。Epic 没有“最近”，因为它的 API 不提供最近游玩数据。Battle.net 和 Riot 只支持“全部”。为某条计划选择的游戏会覆盖其预设。
- **新计划历史。** 新的已保存计划会一直显示“上次运行：从未运行”，直到该计划首次运行完成。下次运行时间按其间隔推算。
- **目标平台。** 只有 Steam 支持 Windows、Linux 和 macOS depot 筛选，默认值为 Windows。
- **下载控制。** 强制重新下载默认关闭。连接数可以是**自动**，也可以固定为 1 到 256。每条计划都可以正常显示运行通知或静默运行。
...
计划任务页面的其余部分，对每个后台服务都采用相同的方式工作——日志轮转、失效扫描、游戏检测、缓存快照等等。每个服务都是一行，各有自己的间隔，行尾带一个运行控件。这里也有一行 **Xbox 游戏映射**，让 Xbox 游戏目录能按自己的计划刷新。
