...
<em>Dashboard - bandwidth saved, hit ratio, service analytics, and top clients in one view</em>
</div>
...
<em>Downloads - every cached game with cover art, sizes, and per-client history</em>
</div>
...
<em>Clients - which devices use the cache, and how well it serves each one</em>
</div>
...
<em>Events - download activity and LAN events on a calendar</em>
</div>
...
<em>Status Check - verify DNS, cache reachability, and recent download routing</em>
</div>
...
<em>Management → Logs & Cache - process logs, manage the disk cache, and detect corrupted or evicted files</em>
</div>
...
<em>Settings - authentication, demo mode, and display preferences.</em>
</div>
...
<em>Integrations - sign in to the game platforms and configure the Prometheus endpoint. One page shows the login state of all five prefill services.</em>
</div>
...
<em>Data - Steam game mapping and database import.</em>
</div>
...
<em>Schedules - every background service on its own interval, each with a Run Now control. The Scheduled Prefill card lives at the bottom of this page and is shown in the Prefill section below.</em>
</div>
...
<em>Theme - switch between installed themes, import community themes, or upload your own.</em>
</div>
...
<em>Clients - give devices friendly names and exclude machines from the stats.</em>
</div>
...
<em>Prefill Sessions - watch live and persistent prefill containers and review past runs.</em>
</div>
