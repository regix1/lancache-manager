...
应用会从两个受信任的来源建立一份简短的 DNS 服务器列表：主机上已配置的 IPv4 解析器，以及对外暴露 DNS 53 端口的正在运行的 Docker 容器。它按顺序询问这些服务器，并在收到“名称不存在”的答复后继续询问下一个，这样只有缓存、没有反向记录的 DNS 就不会挡住真正有记录的服务器。某个服务器返回了真实名称后，下次会优先询问它。应用不会猜测路由器地址、扫描客户端网段，也不会把这些查询发到公共 DNS。

如果**完全没有任何**地址显示名称，通常是因为这些受信任的服务器上都没有反向记录。运行 dnsmasq 的路由器会记录通过 DHCP 分配地址的设备名称，但这些记录仍需能够通过应用允许询问的某一台服务器查询到。
