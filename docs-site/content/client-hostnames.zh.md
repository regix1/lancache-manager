...
应用会向不止一台 DNS 服务器询问每个地址：先是本机网卡上配置的名称服务器（DHCP 下发的 AdGuard、lancache-dns、unbound 等），然后是它已经找到的 lancache DNS，最后是客户端网段上的路由器地址。AdGuard → lancache-dns → unbound 这样的正向链路通常仍然没有反向记录，因为这些记录在 DHCP 服务器上，并不在这条链路里。如果路由器把所有 53 端口查询都重定向到同一台解析器，那么每一跳都是那一台，DHCP 名称仍然必须发布到那里。

如果走完这条链之后网络上**仍然完全没有任何**地址显示名称，通常是因为这些服务器都没有反向记录。运行 dnsmasq 的路由器本身就会记住它通过 DHCP 分配过地址的每一台设备的名称。先确认路由器自己能回答反向查询（见上文）；如果可以，这些名称通常会在整个网络上一次性出现。
