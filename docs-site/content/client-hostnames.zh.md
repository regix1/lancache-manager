...
如果你的网络上**完全没有任何**地址显示名称，而不只是你正在排查的那一个，通常的原因是你的 DNS 服务器从未被告知要向路由器询问反向查询。运行 dnsmasq 的路由器本身就会记住它通过 DHCP 分配过地址的每一台设备的名称。一旦你的 DNS 服务器被设置为向路由器询问反向记录，这些名称通常会在整个网络上一次性出现，而不只是你正在排查的那一个地址。
